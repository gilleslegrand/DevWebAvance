...
class ProduitManager extends Manager
{
    ...
    {
        $db = $this->dbConnect();
        $req = $db->prepare("SELECT p.*, c.categorie FROM tbl_produit AS p INNER JOIN tbl_categorie AS c ON p.id_categorie = c.id_categorie WHERE p.id_categorie = ? ORDER BY p.id_produit");
